...
plot_scatter_pie_map <- function(plot_data, sf_obj) {
  ...
  if (!requireNamespace("scatterpie", quietly = TRUE)) {
    stop("Error: scatterpie not installed.")
  }
  ...
  plt_obj <- ggplot() +
    geom_sf(data = sf_obj) +
    scatterpie::geom_scatterpie(data=pie_data, aes(x=lon, y=lat, r=radius/1.5), cols=hap_names) +
    theme_bw() +
    scatterpie::geom_scatterpie_legend(log10(hap_pie_data$N), breaks=(log10(c(1, 10, 100)) + 1) / 1.5, x=73, y=-30, labeller=function(x) 10 ^ (x*1.5-1), n=3) +
